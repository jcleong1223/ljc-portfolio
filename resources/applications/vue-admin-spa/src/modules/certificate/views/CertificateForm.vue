<template>
	<v-card class="radius-10">
		<v-card-title>
			<span class="text-h5">{{ (action == 'edit') ? 'Edit' : 'New' }} Certificate</span>
		</v-card-title>
...
export default{
	...
	methods: {
		...
		saveModel(item){
			...
			let theApi = (this.action == "new") ? CertificateClient.create(payload) : CertificateClient.update(payload)
			theApi.then((res)=>{
