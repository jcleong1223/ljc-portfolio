<template>
	<v-sheet color="transparent">
		<v-container>
			...
			<v-row align="end">
				<v-col>
					<div class="text-h5 font-weight-bold">Certificates</div>
					<div class="subtitle-2 grey--text">Certificates that registered in this system.</div>
				</v-col>
			</v-row>
			<!--- Search Bar --->
			<v-row
...
export default{
	...
	methods:{
		...
		getResourceData_(){
			...
			CertificateClient.getList(payload).then((res)=>{
		...
		archiveModel_(item){
			...
			CertificateClient.archive(payload).then((res) => {
				this.initialize_();
