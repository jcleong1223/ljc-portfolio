<template>
	<v-sheet color="transparent">
		<v-container>
			...
			<v-row align="end">
				<v-col>
					<div class="text-h5 font-weight-bold">Admin</div>
					<div class="subtitle-2 grey--text">Admins that registered in this system.</div>
				</v-col>
			</v-row>
			<!-- search bar -->
			<v-row
			...
			<v-card class="mt-4" color="transparent" flat>
				...
				>
					...
					<template #[`item.action`]="{ item }">
						<v-btn icon small :to="{ name: 'admin.info', params: { id: item.id } }">
...
export default{
	...
	created(){
		...
		this.options_.admin_type = this.getAdminTypeRefs()
		this.getAllRefs_([
			// async methods
		]).then(()=>{
			this.setupFilterBar()
	...
	methods:{
		...
		getResourceData_(){
			...
			AdminClient.getList(payload).then((res)=>{
		...
		setupFilterBar(){
			this.filter_options = [
				...
				// 	model: "role_name",
				// 	label: "Role Type",
				// 	items: [ { value: null, text: "All" }, ...this.options_.admin_type || [] ],
				// },
				{
					cols: 12,
					model: "activity",
		...
		download(){
			...
			this.model_loading_ = true
			AdminClient.exportList(payload).then((res)=>{
