<template>
	<v-sheet color="transparent">
		<v-container>
			...
			<v-row align="end">
				<v-col>
					<div class="text-h5 font-weight-bold">Fabrications</div>
					<div class="subtitle-2 grey--text">Fabrications that registered in this system.</div>
				</v-col>
			</v-row>
			<!--- Search Bar --->
			<v-row
...
import FabricationForm from "./FabricationForm.vue"
...
export default{
	...
	methods:{
		...
		getResourceData_(){
			...
			FabricationClient.getFabricationList(payload).then((res)=>{
		...
		archiveModel_(item){
			...
			FabricationClient.archiveFabrication(payload).then((res) => {
				this.initialize_();
