...
export default{
	...
	data(){
		return {
			...
			tabs: [
				...
				// { title: 'Orders', to: { name: 'order.list', query: { user_id : this.$route.params.id }} },
			],
	...
	created(){
		...
		this.breadcrumbs_data = [
			{ text: 'User List', disabled: false, to: { name: 'user.list' }, exact: true },
			{ text: 'User Info', disabled: true },
		];
	...
	methods:{
		getResourceData_(){
			UserClient.getInfo(this.infoID).then((res) => {
		...
		// 		token_id : token.id,
		// 	}
		// 	UserClient.revokeSessionToken(payload).then((res) => {
