		...
		<v-container>
			...
			<v-row align="end">
				<v-col>
					...
					<div class="subtitle-2 grey--text">Tags that registered in this system.</div>
				</v-col>
			</v-row>
			<!--- Search Bar --->
			<v-row
...
export default{
	...
	data(){
		return {
			table_headers: [
				{ text: 'Id', value: 'id' },
				{ text: 'Icon image', value: 'web_image' },
	...
	methods:{
		...
		getResourceData_(){
			...
			TagClient.getList(payload).then((res)=>{
		...
		archiveModel_(item){
			...
			TagClient.archive(payload).then((res) => {
				this.initialize_();
